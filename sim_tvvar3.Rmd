...
for (idx in 1:nrow(param_grid)) {
  ...
  filename = sprintf('ks__n_%i_M_%i_nu_%i_L_%i_TV_%0.2f_%s.rds',
  ...
  if (!is.null(overall_accuracy)) {
    result_row = overall_accuracy |>
      select(Method, Perfect_Recovery, Detect_11_TV , Detect_12, Detect_23) |>
      mutate(
        n = n,
        M = M,
        TV_size = TV_size,
...
```{r}
# ============================================================================
# MAIN LOOP - WITH MASTER RESULTS TABLE ONLY (NO INDIVIDUAL FILES)
# ============================================================================
...
for (idx in 1:nrow(param_grid)) {
  ...
  tryCatch({
    ...
    cat(sprintf("Completed iteration %d/%d\n\n", idx, nrow(param_grid)))
...
# Save master results table ONLY
master_results_file = file.path(output_dir, "master_results_table_ks_.6.rds")
saveRDS(master_results, file = master_results_file)

# Also save as CSV for easy viewing
master_results_csv = file.path(output_dir, "master_results_table_ks_.6.csv")
